...
def home(request):
    ...
    last_activity = request.session.get('last_activity', 'No activity recoded!')
...
def set_cookie(request):
    ...
    response.set_cookie('username', 'JohnDoe', max_age=60)
    return response
...
def set_session(request):
    ...
    request.session['lst_activity'] = str(timezone.now())
    return HttpResponse("Sessio data has been set!")
...
def clear_session(request):
    ...
    return HttpResponse("Session has beeb cleared!")
